// 广播厅天花灯盘光池 最终 A/B:组件默认参数 on/off 截图 + base64 像素差分。
// 机位:站立(常规视角)/俯视(地板光池)/沿排(三池纵深)/抬头(天花无烤边)。
...
const discs = await page.evaluate(() => window.__broadcastCeilingLights?.discs ?? null)
...
const views = [
  ...
  { name: 'row', eye: { x: -20.9, y: 1.5, z: 3.4 }, look: { x: -13.0, y: 0.5, z: 3.45 } },
  { name: 'lookup', eye: { x: -16.4, y: 1.6, z: 2.6 }, look: { x: -15.68, y: 5.3, z: 3.43 } },
]

for (const view of views) {
  await page.evaluate(({ eye, look }) => window.__teleport(eye, look), view)
  await settle()
  const onPath = `.tmp-bcl-final-${view.name}-on.png`
  await page.screenshot({ path: onPath, timeout: 120000 })
  await page.evaluate(() => window.__broadcastCeilingLights?.toggle())
  await settle()
  const offPath = `.tmp-bcl-final-${view.name}-off.png`
  await page.screenshot({ path: offPath, timeout: 120000 })
  await page.evaluate(() => window.__broadcastCeilingLights?.toggle())
  console.log(`${view.name}:`, JSON.stringify(await diffB64(readFileSync(offPath).toString('base64'), readFileSync(onPath).toString('base64'))))
}
await browser.close()
